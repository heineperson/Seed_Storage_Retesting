...
germinationSummary <- germCountsbyTest[,.(BaselineYN=ifelse(grepl("Initial",toString(TestType)),"Yes","No"), 
                                       BaselineStatus=toString(Baseline[TestType=="Initial"]),
...
datatable(overallSummary)

```

# Fresh Baseline Priorities 

I'm assuming we don't have fresh seed for anything before 2015, so this list only reflects accessions with no baselines after that point. 

```{r fresh priorities, message=FALSE, warning=FALSE}
datatable(overallSummary[BaselineYN=="No" & DateSeedCollected>"2015-01-01",.(Acc=SeedAccessionNumber,Spp=ScientificName,CNPSRank,LocalityName,Collected=DateSeedCollected,Frozen=DateFrozen)][order(CNPSRank,Collected)])
```
