...
germTestMelt <- melt(germTest, id.vars=c(),measure.vars = names(germTest)[14:23],variable.name="Rep",value.name="InitialCount")
# Remove "_InitialCount" from rep column
germTestMelt[,Rep:=gsub('_.*', '', Rep)]
...
germCountsbyRep = germMerge[,.(InitialCount=InitialCount[1], MaxCount = max(measurementCount,na.rm=T)), by=c("SeedAccessionNumber","PK_ID", "Rep","StartDate","FinalDate","TestType","TestStatus","TestTypeSp")]
germCountsbyTest  = germCountsbyRep[,.(SeedsTested=sum(InitialCount), Germinants = sum(MaxCount), GermPercent=round(sum(MaxCount)/sum(InitialCount)*100)), by=c("SeedAccessionNumber","TestType","PK_ID","StartDate","FinalDate","TestStatus","TestTypeSp")]
